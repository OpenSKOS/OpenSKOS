...
var EditorConceptsSelection = new Class({
    ...
    initialize: function (selectionContainer) {
        ...
        $(document.body).addEvent("click:relay(.add-to-selection)", function (e) {
            e.stop();
            Editor.ConceptsSelection.add(new Element(e.target).getParent('.concept-link').getElement('.uri').get('text'));
        });
    ...
    add: function (uri) {
        var uris = new Array();
        uris.push(uri);
        this.addMultiple(uris);
    },
    addMultiple: function (uris) {
        new Request.JSON({url: this.controllerUrl + 'add', data: {'uris': uris}, onSuccess: this.addReady}).send();
    },
    ...
    remove: function (uri) {
        new Request.JSON({url: this.controllerUrl + 'remove', data: {'uri': uri}, onSuccess: this.removeReady}).send();
    },
    ...
    refreshList: function (concepts) {
        ...
        for (var i = 0; i < concepts.length; i++) {
            ...
            item.getElement('.uri').set('text', concepts[i].uri);
            item.getElement('.remove-link').addEvent('click', function (ev) {
                ev.stop();
                self.remove(new Element(ev.target).getParent('.concept-link').getElement('.uri').get('text'));
            });
    ...
    getUris: function () {
        ...
        for (i = 0; i < items.length; i++) {
            uris.push(items[i].getElement('.uri').get('text'));
        }
        return uris;
    }
});