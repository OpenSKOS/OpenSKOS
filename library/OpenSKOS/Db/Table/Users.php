...
class OpenSKOS_Db_Table_Users extends Zend_Db_Table
{
    ...
    {
        return $usertype ==
                (self::USER_TYPE_BOTH || $usertype == self::USER_TYPE_EDITOR) && Zend_Registry::get(OpenSKOS_Application_Resource_Acl::REGISTRY_KEY)->isAllowed($role, 'editor', 'view');
        ;
    }
    ...
    public function getUrisByRoles($tenants, $roles)
    {
        $select = $this->select()
            ->from($this->_name, ['uri']);
